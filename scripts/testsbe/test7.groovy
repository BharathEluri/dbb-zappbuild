...
def copy = new CopyToPDS().file(new File("/u/adcde/dbb-zappbuild/scripts/cobol/test7.cbl")).dataset("ADCDE.BUILD.COBOL").member("HELLO7")
copy.execute()
...
compile.dd(new DDStatement().name("SYSIN").dsn("ADCDE.BUILD.COBOL(HELLO7)").options("shr"))
compile.dd(new DDStatement().name("SYSLIN").dsn("ADCDE.BUILD.OBJ(HELLO7)").options("shr"))
...
compile.copy(new CopyToHFS().ddName("SYSPRINT").file(new File("/u/adcde/out/test7.log")))
...
link.dd(new DDStatement().name("SYSLIN").dsn("ADCDE.BUILD.OBJ(HELLO7)").options("shr"))
link.dd(new DDStatement().name("SYSPRINT").options("cyl space(5,5) unit(vio) new"))
link.dd(new DDStatement().name("SYSLMOD").dsn("ADCDE.COB.LOADLIB(HELLO7)").options("shr"))
...
link.copy(new CopyToHFS().ddName("SYSPRINT").file(new File("/u/adcde/out/sysprintlink7.log")))
